...
pthread_mutex_t mutex1;//定义一把锁的地址
...
int main(int argc,char*argv[])
{
	...
	*/
	...
	pthread_mutex_init(&mutex1,NULL);//创建一把锁
	pthread_create(&pthid,NULL,show_image,(void*)NULL);//创建一个线程
	//pthread_create(&pthid1,NULL,tcp_ser,(void*)NULL);//创建一个线程
	...
	while(1)
	{
		...
		else  //监听到了 
		{
			if(FD_ISSET(fd,&rfds)) //操作  
			{
		        ...
		        if(ret < 0)
		        {
		            printf("Failed to join the q");
		            pthread_mutex_unlock(&mutex);
		            continue;
		        }
		        pthread_mutex_unlock(&mutex);
		        pthread_mutex_lock(&mutex1);
		        ret=Enqueue(q1,ub[vbuf.index].start);
		        if(ret < 0)
		        {
		            printf("Failed to join the q1");
		            pthread_mutex_unlock(&mutex1);
		            continue;
		        }
		        pthread_mutex_unlock(&mutex1);
